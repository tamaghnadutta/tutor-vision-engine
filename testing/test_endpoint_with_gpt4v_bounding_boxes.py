#!/usr/bin/env python3
"""
Test the /detect-error endpoint using aiohttp with GPT-4V determined bounding boxes
"""
...
def load_gpt4v_test_cases() -> List[Dict[str, Any]]:
    """Load test cases generated by GPT-4V analysis"""
    try:
        with open("gpt4v_test_cases.json", "r") as f:
            return json.load(f)
    except FileNotFoundError:
        print("❌ GPT-4V test cases not found. Run analyze_bounding_boxes_gpt4v.py first.")
        return []

async def test_detect_error_endpoint(session: aiohttp.ClientSession, test_case: Dict[str, Any]) -> Dict[str, Any]:
    """
    Test the detect-error endpoint with a GPT-4V generated test case
    """
    print(f"\n🧪 Testing: {test_case['name']}")
    print(f"   Description: {test_case['description']}")

    bbox = test_case['bounding_box']
    print(f"   Bounding Box: ({bbox['minX']:.2f}, {bbox['minY']:.2f}) → ({bbox['maxX']:.2f}, {bbox['maxY']:.2f})")
    ...
    try:
        async with session.post(
            ...
            if response.status == 200:
                ...
                if result.get('error'):
                    print(f"   🚨 Error found: {result['error'][:100]}{'...' if len(result['error']) > 100 else ''}")

                if result.get('y'):
                    print(f"   📍 Error Y position: {result['y']}")

                if result.get('correction'):
                    print(f"   🔧 Correction: {result['correction'][:100]}{'...' if len(result['correction']) > 100 else ''}")
...
async def test_selected_cases():
    """
    Run a curated selection of the most interesting GPT-4V test cases
    """
    print("🚀 TESTING DETECT-ERROR ENDPOINT WITH GPT-4V BOUNDING BOXES")
    print("=" * 80)
    ...
    print(f"📋 Selected {len(selected_cases)} most relevant test cases:")
...
async def main():
    ...
    if results:
        ...
        print(f"\n🎉 GPT-4V bounding box testing completed!")
